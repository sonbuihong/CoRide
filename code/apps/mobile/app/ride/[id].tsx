...
function PassengerRideView() {
  ...
  const { RideMap } = require('../../src/components/RideMap');
  ...
  if (!ride) {
    return (
      <View style={styles.centeredPad}>
        ...
        <AppButton title="Quay lại" variant="passenger" onPress={() => router.back()} style={styles.mt4} />
      </View>
    );
  }
  ...
  return (
    <View style={styles.passengerContainer}>
      {/* Header nổi */}
      <View style={[styles.passengerHeaderFloat, { paddingTop: insets.top + 10 }]}>
        <Pressable
          onPress={() => router.back()}
          style={({ pressed }) => [styles.passengerFloatBtn, pressed && styles.pressed]}
          accessibilityRole="button"
      ...
      <ScrollView style={styles.passengerScroll} showsVerticalScrollIndicator={false}>
        <View style={styles.passengerMapWrap}>
          <RideMap departureCoords={ride.departureCoords} destinationCoords={ride.destinationCoords} />
        </View>

        <View style={styles.passengerContent}>
          {/* Thông tin điểm đi - điểm đến chính */}
          <View style={styles.passengerRouteCard}>
            <View style={styles.passengerRouteTop}>
              <View style={styles.flex1}>
                <AppText variant="h2" weight="bold" style={styles.passengerDest}>Đến: {ride.destination}</AppText>
                <AppText variant="bodySmall" style={styles.passengerDepText}>
                  Từ: <AppText weight="bold" style={styles.textPrimary}>{ride.departure}</AppText>
                </AppText>
              </View>
              <AppText variant="h2" weight="bold" style={styles.passengerPrice}>
                {(ride.price ?? 0).toLocaleString('vi-VN')}đ
              </AppText>
            </View>
            <AppText variant="caption" weight="medium" style={styles.passengerDate}>
          ...
          {matchingRide.matchScore != null && (
            <View style={styles.mb6}>
              <AppText variant="body" weight="semibold" style={styles.sectionTitlePassenger}>
          ...
          {ride.stops && ride.stops.length > 0 && (
            <View style={styles.mb6}>
              <AppText variant="body" weight="semibold" style={styles.sectionTitlePassenger}>Chọn điểm đón</AppText>
              <AppText variant="bodySmall" style={styles.passengerHint}>
                Bạn có thể bắt đầu tại điểm đi chính hoặc một điểm đón công khai dọc tuyến.
              </AppText>
              <View style={styles.passengerStopsCard}>
                <PickupChoice
                  title="Điểm đi chính"
          ...
          <View style={styles.passengerInfoGrid}>
            <View style={[styles.passengerInfoCell, styles.passengerInfoCellBorder]}>
              <Clock size={20} color="#3B82F6" />
              <AppText variant="caption" style={styles.passengerInfoLabel}>Giờ xuất phát</AppText>
              <AppText variant="bodySmall" weight="bold" style={styles.textPrimary}>
                {format(new Date(ride.departureTime), 'HH:mm')}
              </AppText>
            </View>
            <View style={[styles.passengerInfoCell, styles.passengerInfoCellBorder]}>
              <Users size={20} color="#3B82F6" />
              <AppText variant="caption" style={styles.passengerInfoLabel}>Ghế trống</AppText>
              <AppText variant="bodySmall" weight="bold" style={styles.textPrimary}>
                {ride.availableSeats} / {ride.totalSeats ?? ride.availableSeats}
              </AppText>
            </View>
            <View style={styles.passengerInfoCell}>
              <ShieldCheck size={20} color="#16A34A" />
              <AppText variant="caption" style={styles.passengerInfoLabel}>Cam kết</AppText>
              <AppText variant="bodySmall" weight="bold" style={styles.passengerSafeText}>An toàn P2P</AppText>
            </View>
          </View>

          {/* DriverTrustCard */}
          <AppText variant="body" weight="bold" style={styles.passengerDriverLabel}>Tài xế của bạn</AppText>
          <View style={styles.passengerDriverCard}>
            <View style={styles.passengerDriverAvatar}>
              {ride.driver?.avatar ? (
                <Image source={{ uri: ride.driver.avatar }} style={styles.passengerAvatarImg} />
              ) : (
                <AppText variant="h2" weight="bold" style={styles.passengerAvatarInitial}>
                  {(ride.driver?.firstName ?? '?').charAt(0)}
                </AppText>
              )}
            </View>
            <View style={styles.flex1}>
              <AppText variant="body" weight="bold" style={styles.textPrimary}>
                {ride.driver?.firstName} {ride.driver?.lastName}
              </AppText>
              <View style={styles.passengerRatingRow}>
                <Star size={14} color="#F59E0B" fill="#F59E0B" />
                <AppText variant="caption" weight="bold" style={styles.passengerRatingText}>
                  {(ride.driver?.rating ?? 5).toFixed(1)}
                </AppText>
                <ShieldCheck size={12} color="#16A34A" />
                <AppText variant="caption" style={styles.passengerVerifiedText}>Tài xế uy tín</AppText>
              </View>
            </View>
            <Pressable
              style={({ pressed }) => [styles.passengerChatBtn, pressed && styles.pressed]}
              accessibilityRole="button"
      ...
      {/* Sticky Bottom */}
      <View style={[styles.passengerCta, { paddingBottom: insets.bottom + 16 }]}>
        <View style={styles.passengerSeatsRow}>
          <View style={styles.flex1}>
            <AppText variant="bodySmall" weight="bold" style={styles.textPrimary}>Số lượng ghế cần đặt</AppText>
            <AppText variant="caption" style={styles.passengerSeatsHint}>
              Mỗi hành khách đặt tối đa {ride.availableSeats} ghế
            </AppText>
          </View>
          <View style={styles.passengerSeatsPicker}>
            <Pressable
              onPress={() => setSeats(Math.max(1, seats - 1))}
              style={({ pressed }) => [styles.passengerSeatsBtn, pressed && styles.pressed]}
              accessibilityRole="button"
              accessibilityLabel="Giảm số ghế"
            >
              <AppText weight="bold" style={styles.textPrimary}>-</AppText>
            </Pressable>
            <AppText weight="bold" style={styles.passengerSeatsCount}>{seats}</AppText>
            <Pressable
...
// ─── Passenger PickupChoice (giữ nguyên) ──────────────────────────────────────
