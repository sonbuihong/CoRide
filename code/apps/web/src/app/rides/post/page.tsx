...
// Component điểm dừng: mặc định là 1 dòng gọn, nhấn vào mới mở ra form đầy đủ
function StopsSection({
  ...
  onUpdate: (index: number, patch: Partial<StopItem>) => void;
  onMove: (from: number, to: number) => void;
}) {
  const [panelOpen, setPanelOpen] = useState(stops.length > 0);

  // Khi có stops mới được thêm, tự mở panel
  const prevLength = React.useRef(stops.length);
  React.useEffect(() => {
    if (stops.length > prevLength.current) setPanelOpen(true);
    prevLength.current = stops.length;
  }, [stops.length]);

  // Đóng panel và xóa tất cả stops
  const handleCloseAll = () => {
    // Xóa ngược từ cuối để index không bị lệch
    for (let i = stops.length - 1; i >= 0; i--) onRemove(i);
    setPanelOpen(false);
  };

  if (!panelOpen) {
    // Chế độ thu gọn: 1 dòng duy nhất
    return (
      <button
        type="button"
        onClick={() => { setPanelOpen(true); onAdd(); }}
        className="flex min-h-11 w-full items-center gap-2.5 rounded-xl border border-dashed border-black/15 bg-transparent px-4 text-left text-xs font-semibold text-black/50 transition-colors hover:border-[#f59e0b]/50 hover:bg-[#f59e0b]/[0.03] hover:text-[#b45309] dark:border-white/10 dark:text-white/40 dark:hover:border-[#f59e0b]/40 dark:hover:text-[#fbbf24]"
        aria-label="Thêm điểm dừng dọc đường"
      >
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border border-black/10 bg-white dark:border-white/10 dark:bg-white/[0.06]">
          <Plus className="h-3.5 w-3.5 text-[#f59e0b]" />
        </span>
        <Route className="h-3.5 w-3.5 shrink-0 text-[#f59e0b]" />
        <span>Thêm điểm dừng dọc đường</span>
        <span className="ml-auto text-[11px] font-normal text-black/30 dark:text-white/30">Tuỳ chọn · tối đa 3 điểm</span>
      </button>
    );
  }

  // Chế độ mở rộng: panel đầy đủ
  return (
    <div className="space-y-3 rounded-2xl border border-black/[0.06] bg-black/[0.015] p-4 dark:border-white/[0.08] dark:bg-white/[0.02]">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Nút X toàn bộ — đóng panel và xóa tất cả */}
        <button
          type="button"
          onClick={handleCloseAll}
          title="Hủy tất cả điểm dừng"
          aria-label="Hủy tất cả điểm dừng và thu gọn"
          className="flex h-7 w-7 items-center justify-center rounded-lg text-black/40 transition-colors hover:bg-[#ff3b30]/10 hover:text-[#ff3b30] dark:text-white/35 dark:hover:bg-[#ff3b30]/20 dark:hover:text-[#ff453a]"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      {stops.map((stop, index) => {
        ...
        const isExpanded = stop._expanded !== false;
        ...
        if (!isExpanded && hasCoords) {
          ...
          return (
            <button
              key={stop.id}
              type="button"
              onClick={() => onUpdate(index, { _expanded: true } as Partial<StopItem>)}
            ...
            >
              ...
              <div className="flex shrink-0 items-center gap-0.5">
                ...
                {/* Nút X từng điểm — chế độ thu gọn */}
                <button
                  type="button"
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                ...
                {/* Thu gọn về 1 dòng (chỉ khi đã có tọa độ) */}
                {hasCoords && (
                  <button
                    type="button"
                    onClick={() => onUpdate(index, { _expanded: false } as Partial<StopItem>)}
                ...
                {/* Nút X từng điểm — chế độ mở rộng */}
                <button
                  type="button"
            ...
            <div className="flex items-center justify-between rounded-lg bg-black/[0.03] px-3 py-2 dark:bg-white/[0.04]">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  onClick={() => onUpdate(index, { waitTimeMinutes: Math.max(1, stop.waitTimeMinutes - 1) })}
                ...
                <button
                  ...
                  onClick={() => onUpdate(index, { waitTimeMinutes: Math.min(15, stop.waitTimeMinutes + 1) })}
                  className="flex h-7 w-7 items-center justify-center rounded-md border border-black/10 bg-white text-xs font-semibold shadow-xs transition-colors hover:bg-black/[0.05] disabled:cursor-not-allowed disabled:opacity-30 dark:border-white/10 dark:bg-[#1c1c1e] dark:hover:bg-white/[0.08]"
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.25fr)_minmax(280px,0.75fr)]">
      <Surface>
        ...
        <div className="space-y-5">
          ...
          {/* Điểm dừng dọc đường (Waypoints) — thu gọn mặc định */}
          <StopsSection
            stops={stops}
            onAdd={handleAddStop}
