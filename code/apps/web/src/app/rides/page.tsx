    ...
    <div className="flex h-screen bg-white dark:bg-black font-sans overflow-hidden w-full">
      ...
      <div className="w-full md:w-[380px] lg:w-[420px] flex flex-col border-r border-gray-200 dark:border-gray-800 bg-white dark:bg-black h-full flex-shrink-0 shadow-sm z-10">
        ...
        <div className="px-4 py-2 flex flex-col gap-3">
          ...
          <div className="flex items-center gap-3 w-full border-[1.5px] border-[#0071e3] rounded-[20px] h-[52px] px-4 bg-white dark:bg-gray-900 shadow-[0_0_0_4px_rgba(0,113,227,0.05)] focus-within:ring-2 focus-within:ring-[#0071e3]/20">
            ...
            <input 
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  router.push(`/rides/search?origin=${encodeURIComponent(originValue)}&destination=${encodeURIComponent(destinationValue)}`);
                }
              }}
          ...
          <button 
            onClick={() => router.push(`/rides/search?origin=${encodeURIComponent(originValue)}&destination=${encodeURIComponent(destinationValue)}`)}
            className="w-full h-[52px] bg-[#0071e3] text-white rounded-[20px] font-semibold text-[16px] shadow-sm hover:bg-[#0077ed] transition-colors"
          >
            Tìm chuyến đi
          </button>
        </div>
        ...
        <div className="px-3 py-2 flex flex-col gap-1.5 flex-1 overflow-y-auto">
           {/* Desktop Only Button */}
           <button className="hidden md:flex items-center gap-4 w-full bg-[#f5f8fa] dark:bg-gray-800 rounded-[20px] h-[52px] px-4">
           ...
           {/* Mobile Only List */}
           <div className="md:hidden flex flex-col gap-2 mb-2 mt-2">
              <h3 className="text-[15px] font-bold text-[#1d1d1f] dark:text-white px-2 mb-1">Địa điểm gần đây & Yêu thích</h3>
              {mockLocations.map(loc => (
                 <div key={loc.id} className="flex items-center p-3 sm:p-4 bg-white dark:bg-[#1c1c1e] border border-gray-100 dark:border-gray-800 rounded-[20px] hover:shadow-sm active:scale-[0.98] transition-all cursor-pointer shadow-sm">
                    {/* Icon Area */}
                    ...
                    <div className="flex-1 pr-2">
                       ...
                       <p className="text-[12px] text-gray-500 line-clamp-1 leading-snug">{loc.address}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                       ...
                       <button className="h-8 w-8 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                          <Heart className="h-[16px] w-[16px] text-gray-400" strokeWidth={1.5} />
                       </button>
                    </div>
                 </div>
              ))}
