...
import { ArrowLeft, Bike, Car, Check, LocateFixed, Loader2, MapPin, Navigation, Route, ShieldCheck, AlertCircle } from 'lucide-react';
...
import { formatEta, formatPrice, formatTripDistance, isTerminalTripStatus, type Coordinates, type TripEstimate, type VehicleType } from './domain';
...
export default function BookingExperience() {
  ...
  // Kiểm tra nếu hành khách đang có chuyến đi chưa kết thúc
  const activeTripQuery = useQuery({
    queryKey: passengerTripKeys.active,
    queryFn: () => passengerTripService.active(),
    enabled: Boolean(user),
    staleTime: 5_000,
  });
  const activeTrip = activeTripQuery.data;
  const hasActiveTrip = Boolean(activeTrip && !isTerminalTripStatus(activeTrip.status));
  ...
  const createTrip = useMutation({
    mutationFn: () => {
      ...
      return passengerTripService.create(draft);
    },
    onSuccess: (trip) => {
      sessionStorage.setItem('coride-active-trip-id', trip.id);
      draft.reset();
      router.push(`/ride-hailing/searching?tripId=${trip.id}`);
    },
    onError: (error: unknown) => {
      const isTripExists = error instanceof Error && error.message === 'ACTIVE_TRIP_EXISTS';
      if (isTripExists) {
        toast.error('Bạn đang có chuyến xe chưa kết thúc. Vui lòng kiểm tra lại chuyến hiện tại.');
      } else {
        toast.error('Không thể đặt xe. Vui lòng kiểm tra thông tin và thử lại.');
      }
    },
  });
    ...
    <section className="absolute inset-x-0 bottom-0 z-20 flex max-h-[66dvh] flex-col rounded-t-[22px] bg-white shadow-[2px_-8px_32px_rgba(24,45,29,.16)] md:bottom-6 md:left-6 md:right-auto md:top-6 md:max-h-none md:w-[440px] md:rounded-[16px]">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 pb-4 pt-4 md:px-6">
        ...
        {hasActiveTrip && activeTrip && (
          <div className="mb-5 flex items-center justify-between gap-3 rounded-[14px] border border-emerald-300 bg-emerald-50 p-3.5 text-emerald-950 shadow-sm">
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5 font-semibold text-[#16833b]">
                <AlertCircle className="h-4 w-4 shrink-0" />
                <span className="text-xs uppercase tracking-wide">Chuyến xe đang diễn ra</span>
              </div>
              <p className="mt-0.5 truncate text-xs text-[#2c3e30]">
                Mã: {activeTrip.id.slice(0, 8)}... ({activeTrip.status})
              </p>
            </div>
            <Link
              href={`/ride-hailing/searching?tripId=${activeTrip.id}`}
              className="flex shrink-0 items-center gap-1 rounded-[10px] bg-[#16833b] px-3 py-1.5 text-xs font-semibold text-white shadow hover:bg-[#116c30]"
            >
              Xem chuyến
            </Link>
          </div>
